...
#ifdef INCLUDE_DHT
/*Add DHT to data stream, you need this library https://github.com/adafruit/DHT-sensor-library
...
void InitADC(void){
  ...
  ADCPacer_Timer.initialize(100000);    // The base burst throughput rate is 10s/s due to DHT blocks interrupts, and this is the skew between channels
  ...
  { 
    ...
    if (adcDecCounter[ch]!=0){
      ...
      if ((channellist[ch]&0xF)>=MAXADCHANNEL){
         ...
         fadc_reg=DHTdata[(channellist[ch]&0xF)-MAXADCHANNEL];
      }
      #endif
      ...
      if (ADCChannelIdx==ADCChannelCount)
        ...
        if (ADCChannelIdx==ActualChannelCount){
          while (ADCChannelIdx<ADCChannelCount){
            ...
            if (dqWindaq){ /*Patch blank data*/
            #ifdef INCLUDE_DHT  
              adc_reg=DHTdata[channellist[(ADCChannelIdx&0xF)]-MAXADCHANNEL]^DQ_INVERTSIGN;
            #else
              adc_reg=0;
            #endif                
              ADCdata[wADCdataIdx++]=(adc_reg>>1)|1;
              ...
              #ifdef INCLUDE_DHT   
                adc_reg=DHTdata[channellist[(ADCChannelIdx&0xF)]-MAXADCHANNEL];
              #else
                adc_reg=0;
              #endif  
              if ((dqMode&0x7f)==0){
...
float findTrueSampleRate (float f)
{
  ...
  if (f>10)f=10; //DHT blocks interrupt 

  if (ActualChannelCount==0)ActualChannelCount=1;
  long temp=(long)(10./(f*(float)ActualChannelCount));

  if (temp<1)temp=1;
  if (temp>60000)temp=60000;

  r=10/(((float)temp*(float)ActualChannelCount));
